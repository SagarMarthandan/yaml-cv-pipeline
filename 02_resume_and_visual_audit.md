...
## Inputs
- **Base Resume:** Loaded from `C:\Users\sagar\Documents\YAML-CV\Base Files\<Language>\` (`resume.md` or `resume_de.md`).
...
- Apply the **Stop-Slop** rules (strict active voice, absolute adverb ban, zero em-dashes, no filler jargon).
...
3. **Format:** Replace the project title, tools, and itemize blocks with:
   ```latex
   \noindent\textbf{[Project Name]} --- [Action verb] [what was built] [quantified metric] [tools woven in]. [Second/third sentences with detail/tools/outcomes/CI/CD].
   ```
...
```latex
\noindent\textbf{Project Name} --- ELT pipeline ingesting 37M records into BigQuery via Airbyte, orchestrated with Airflow DAGs on GCP. Modeled dbt mart layers with incremental materialization delivering KPIs. GCP provisioned via Terraform with GitHub Actions CI/CD.
```
> **Spacing note:** Do NOT add `\vspace{...}` at the end of individual project paragraphs. Inter-project spacing is controlled by the `\vspace{6pt}` separator generated by the compiler between each project block, ensuring uniform gaps throughout the section.
...
contact_info:
  ...
  photo_path: "C:/Users/sagar/Documents/YAML-CV/Base Files/Photo/Sagar.jpg"
...
[PASTE TAILORED PROJECT PORTFOLIO FROM local project_info.md HERE]
[PASTE STEP 1 OUTPUT (ATS_Report.yaml) HERE]
