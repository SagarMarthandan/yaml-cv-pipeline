...
```latex
\noindent\textbf{Project Name} --- ELT pipeline ingesting 37M records into BigQuery via Airbyte, orchestrated with Airflow DAGs on GCP. Modeled dbt mart layers with incremental materialization delivering KPIs. GCP provisioned via Terraform with GitHub Actions CI/CD.\vspace{6pt}
```
